...
> `_manual/SlayTheSpire2ModdingTutorials/` 里的官方中文教程已经讲得很清楚了（推荐先读
> `Basics/01-环境配置`、`BaseLib/01-添加卡牌`、`Basics/05-变量与描述`、`Basics/07-快速调试&热重载`）。
>
> **配套资料**：本教程的"词典版"是同目录的 [`sts2-official-effects-survey.md`](./sts2-official-effects-survey.md)（下称《参考手册》）。
> 教程教你写，手册供你查——每个例子末尾的「查深」都指向手册的具体章节。
>
> 文中所有中文术语（人工制品、集中、奇巧、辉星……）均为**游戏官方简中本地化用词**（出处见《参考手册》附录 C）。
...
```csharp
// ① 类与继承：我的卡 = 一个继承 CardModel（本项目里是 MwCardModel）的类
public sealed class MwFoo() : MwCardModel(1, CardType.Attack, CardRarity.Common, TargetType.AnyEnemy)
...
### 0.3 本项目的基类与注册约定（30 秒版）

| 内容 | 怎么写 | 要不要特性 |
|---|---|---|
| 新卡牌（普通） | `class MwXxx() : MwCardModel(费用, CardType.X, CardRarity.X, TargetType.X)` | ❌ 基类已带 `[RegisterCard(..., Inherit = true)]`，子类自动入池 |
| 初始卡组里的卡 | 同上，再加 `[RegisterCharacterStarterCard(typeof(MoWang), 数量)]` | ✅ |
| 音符/弃置主题卡 | 继承 `MwHarmonyCardModel` / `MwDiscCardModel`（其余完全相同） | ❌ |
| 新遗物 | `class MwXxx() : MwRelicModel(RelicRarity.Uncommon)` | ❌ 同上自动注册 |
| 新药水 | `class MwXxx() : MwPotionModel(PotionRarity.X, PotionUsage.X, TargetType.X)` | ❌ |
| 新 Power（增益/减益） | `class MwXxxPower : ModPowerTemplate` | ✅ `[RegisterPower]` |

本地化键：类名转大写下划线（`MwStrike` → `SLAY_THE_STELLA_CARD_MW_STRIKE.title/.description`）；
卡图放 `SlayTheStella/images/cards/{类名}.png` 即被自动找到。
改完跑 `pwsh ./Tools/Check-ContentConsistency.ps1` 查漏。
...
完整 24 个 Var 见《参考手册》§4.4。需要**运行时计算**的数值（"每有一张 X 牌 +2"这类）不要硬凑静态 Var，用下一节的 `ComputedDynamicVar`。
...
### 1.3 计算型数值：ComputedDynamicVar（本项目首选）

当数值需要**运行时计算**——依赖目标（"+目标身上的力量层数"）、战斗状态（"每有一张打击牌 +2"）、
其他变量、甚至预览模式——静态 Var 就不够了。官方原版有 `CalculatedDamageVar` +
`CalculationBaseVar/CalculationExtraVar` 三件套（《参考手册》§4.4，读官方卡代码时能认出来即可），
但设计繁琐；**本项目（RitsuLib）一律用 `ModCardVars.Computed*` 系列**：传一个委托，显示值与结算值
运行时直接计算。
...
> 完整 API（独立预览工厂、共享 tooltip `WithSharedTooltip`、ctx 成员总表）见官方教程
> `RitsuLib/01 - 添加基础内容/19 - 计算动态变量`。
...
> `TargetType` 怎么选：单体攻击 `AnyEnemy`、全体 `AllEnemies`、纯自我 `Self`（完整枚举见《参考手册》§2.3）。

### 2.1 造成伤害（单体）

```csharp
/// <summary>造成 6 点伤害。</summary>
public sealed class MwFooStrike() : MwCardModel(1, CardType.Attack, CardRarity.Common, TargetType.AnyEnemy)
{
    protected override IEnumerable<DynamicVar> CanonicalVars => [new DamageVar(6m, ValueProp.Move)];
...
```csharp
public sealed class MwFooMultiHit() : MwCardModel(1, CardType.Attack, CardRarity.Common, TargetType.AnyEnemy)
{
    protected override IEnumerable<DynamicVar> CanonicalVars =>
    [
...
```csharp
public sealed class MwFooAoe() : MwCardModel(1, CardType.Attack, CardRarity.Common, TargetType.AllEnemies)
{
    protected override IEnumerable<DynamicVar> CanonicalVars =>
    [
...
```csharp
public sealed class MwFooXCost() : MwCardModel(0, CardType.Attack, CardRarity.Uncommon, TargetType.AllEnemies)
{
...
```csharp
public sealed class MwFooDefend() : MwCardModel(1, CardType.Skill, CardRarity.Common, TargetType.Self)
{
...
```csharp
public sealed class MwFooBuff() : MwCardModel(1, CardType.Power, CardRarity.Uncommon, TargetType.Self)
{
...
```csharp
public sealed class MwFooStatus() : MwCardModel(-1, CardType.Status, CardRarity.Status, TargetType.None)
{
...
> 所有 11 张官方手牌触发牌都是状态/诅咒（《参考手册》§5.7-E4）。
...
三个家族（《参考手册》§2.2 有全部 101 个通知钩子的分组总表）：
...
public sealed class MwFooRelic() : MwRelicModel(RelicRarity.Common)
{
...
public sealed class MwFooAggroRelic() : MwRelicModel(RelicRarity.Common)
{
...
public sealed class MwFooDrawRelic() : MwRelicModel(RelicRarity.Uncommon)
{
...
全部 39 个修改器清单见《参考手册》§2.2.8。官方"副作用遗物"——灵体外质（Ectoplasm）：+1 能量/金币归零
...
public sealed class MwFooCounterRelic() : MwRelicModel(RelicRarity.Uncommon)
{
...
> setter 里改值后记得 `InvokeDisplayAmountChanged()`。更完整的范式（setter 内同步 DynamicVar 等）见《参考手册》§8.3。
...
[RegisterPower]
public sealed class MwFooAuraPower : ModPowerTemplate
{
...
Power 的类名会直接成为卡面占位符（`PowerVar<MwFooAuraPower>` → `{MwFooAuraPower}`），
所以**类名起好一点**。Power 本地化在 `powers` 表，键格式与卡牌同理——本项目首个 Power 落地时记得补建
`localization/*/powers.json` 并核对一次实际键名。
...
[RegisterPower]
public sealed class MwFooMightPower : ModPowerTemplate
{
...
>
> 都是官方验证过的实现模式，代码为压缩摘录。查更细的原理：《参考手册》对应章节。
...
```csharp
public sealed class MwFooPerfected() : MwCardModel(1, CardType.Attack, CardRarity.Uncommon, TargetType.AnyEnemy)
{
    protected override IEnumerable<DynamicVar> CanonicalVars =>
    [
...
CalculationBase/ExtraVar 三件套能实现同样效果（《参考手册》§4.4），但 RitsuLib 文档明确不建议再用——
...
public sealed class MwFooMurder() : MwCardModel(3, CardType.Attack, CardRarity.Rare, TargetType.AnyEnemy)
{
    protected override IEnumerable<DynamicVar> CanonicalVars =>
    [
...
```csharp
public sealed class MwFooClaw() : MwCardModel(0, CardType.Attack, CardRarity.Common, TargetType.AnyEnemy)
{
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        foreach (MwFooClaw c in Owner.PlayerCombatState.AllCards.OfType<MwFooClaw>())
        {
...
> 跨战斗还要保留的（ GeneticAlgorithm 式），再叠一层 `[SavedProperty]` + 写回 `DeckVersion`（《参考手册》§5.3-A7）。
...
public sealed class MwFooPinpoint() : MwCardModel(3, CardType.Attack, CardRarity.Uncommon, TargetType.AnyEnemy)
{
...
public sealed class MwFooTutor() : MwCardModel(1, CardType.Skill, CardRarity.Rare, TargetType.AnyAlly)
{
...
public sealed class MwFooBolas() : MwCardModel(0, CardType.Attack, CardRarity.Rare, TargetType.AnyEnemy)
{
...
> 不落库、随来源存在而存在，如 Hex 系），见《参考手册》§2.2.9。移除关键词用 `CardCmd.RemoveKeyword`。
...
```csharp
// 快照目标身上的全部减益（临时 Power 的合并细节见《参考手册》§5.5-B5 的 Misery 拆解）
...
```csharp
public sealed class MwFooFairy() : MwPotionModel(PotionRarity.Rare, PotionUsage.Automatic, TargetType.Self)
{
...
`Kill(force: true)` 连免死保护都能一起绕过（《参考手册》§3.4）。

#### 4.4.3 清空敌人的格挡与人工制品（暴露 Expose 式）

```csharp
public sealed class MwFooExpose() : MwCardModel(0, CardType.Skill, CardRarity.Uncommon, TargetType.AnyEnemy)
{
...
> 人工制品的抵消发生在系统层（《参考手册》§6.1），你的 Apply 不会报错，只会静默无效。
...
protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
{
    ...
    // 反向操作——处决自己的奥斯提换收益：CreatureCmd.Kill（BoneShards/Sacrifice 式，见《参考手册》§5.8-S2）
}
```
...
```csharp
public class MwFooTickPower : ModPowerTemplate
{
    public override PowerType Type => PowerType.Buff;
...
> `AfterPlayerTurnStart`（见《参考手册》§2.2.2）。多回合递减用 Counter 层数，无限循环用 §4.1.5 的仪式式。
...
```csharp
public sealed class MwFooQuestA() : MwCardModel(-1, CardType.Quest, CardRarity.Quest, TargetType.Self)
{
...
```csharp
public sealed class MwFooQuestB() : MwCardModel(-1, CardType.Quest, CardRarity.Quest, TargetType.None)
{
...
| 12 | 本地化词不对 | 用官方名：人工制品/集中/奇巧/固有/虚无/灾厄/覆甲/仪式/辉星/奥斯提（见《参考手册》附录 C） |
...
| 需要什么 | 去哪 |
|---|---|
| 本篇没展开的某个效果细节 | 《参考手册》对应章（§2 钩子总表 / §3 命令 API / §4 数值 / §5 卡牌 / §6 Power / §7 药水 / §8 遗物） |
| 某个钩子/命令的完整签名与官方注释 | `_manual/sts2_export/src/Core/`（注释完整，直接读） |
| "某个钩子有哪些官方在用" | 《参考手册》附录 A 的 grep 命令集 |
| 官方三语译名（写本地化） | `_manual/sts2_export/localization/{zhs,eng,jpn}/`（powers/cards/card_keywords 等 JSON） |
| mod 搭建 / 调试 / 热重载 / 控制台发卡 | `_manual/SlayTheSpire2ModdingTutorials/Basics/`（01 环境、07 调试、09 控制台指令） |
| 项目一致性检查 | `pwsh ./Tools/Check-ContentConsistency.ps1` |
